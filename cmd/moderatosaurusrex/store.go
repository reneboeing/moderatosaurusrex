...
type event struct {
	ID, GuildID, CreatorID, Title, Visibility, InviteCode, GameServer, Species string
	StartsAt                                                                   time.Time
}
...
func scanEvent(row pgx.Row) (event, error) {
	var e event
	err := row.Scan(&e.ID, &e.GuildID, &e.CreatorID, &e.Title, &e.Visibility, &e.InviteCode, &e.GameServer, &e.Species, &e.StartsAt)
	return e, err
}
func (a *app) eventByID(ctx context.Context, guildID, id string) (event, error) {
	return scanEvent(a.pool.QueryRow(ctx, `SELECT id,guild_id,creator_id,title,visibility,COALESCE(invite_code,''),game_server,species,starts_at FROM events WHERE guild_id=$1 AND id=$2`, guildID, id))
}
func (a *app) eventByCode(ctx context.Context, guildID, code string) (event, error) {
	return scanEvent(a.pool.QueryRow(ctx, `SELECT id,guild_id,creator_id,title,visibility,COALESCE(invite_code,''),game_server,species,starts_at FROM events WHERE guild_id=$1 AND invite_code=$2`, guildID, code))
}
func (a *app) publicEvents(ctx context.Context, guildID string) ([]event, error) {
	rows, err := a.pool.Query(ctx, `SELECT id,guild_id,creator_id,title,visibility,COALESCE(invite_code,''),game_server,species,starts_at FROM events WHERE guild_id=$1 AND visibility='public' ORDER BY starts_at LIMIT 10`, guildID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (a *app) participantEvents(ctx context.Context, guildID, userID string) ([]event, error) {
	rows, err := a.pool.Query(ctx, `SELECT e.id,e.guild_id,e.creator_id,e.title,e.visibility,COALESCE(e.invite_code,''),e.game_server,e.species,e.starts_at FROM events e JOIN event_participants p ON p.event_id=e.id WHERE e.guild_id=$1 AND p.user_id=$2 AND p.is_creator=false ORDER BY e.starts_at LIMIT 25`, guildID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (a *app) archiveExpired(ctx context.Context) error {
	_, err := a.pool.Exec(ctx, `DELETE FROM events WHERE starts_at+INTERVAL '8 hours'<=NOW()`)
	return err
}
